...
# funcioens asignatura

def dificultad():
    return random.randint(1, 10)


def cod_titul_asig(tit):
    return tit['cod_titul']


def cod_asig(tit, n_asig):
...
def nota_med_bas(alumno):
    return round(max(5, min(10, random.uniform(-2, 2) + alumno['nota_pred'])), 2)


def nota_med_esp(alumno):
    return round(max(5, min(10, random.uniform(-2, 2) + alumno['nota_pred'])), 2)


def nota_bach(alumno):
    return round(max(5, min(10, random.uniform(-2, 2) + alumno['nota_pred'])), 2)


def nota_acceso(acceso):
...
def cod_titul_cal(alum):
    return simulacion['titulacion'][alum['titulacion']]['cod_titul']


def cod_prof_cal(asig):
    return simulacion['profesor'][asig['profesor']]['cod_prof']


def cod_alu_cal(alum):
    return alum['cod_alu']


def cod_asig_cal(asig):
    return asig['cod_asig']


def convocatoria(conv):
    return conv


def calif_num(asig, alum):
    inc = random.uniform(0, 3)
    prob = asig['dificultad']/10

    return round(min(10, alum['nota_pred'] + inc) if random.uniform(0, 1) > prob else max(0, alum['nota_pred'] - inc), 2)
...
def genera_titul():
    ...
    for tit in range(1, n_titulaciones+1):
        ...
        for campo in jsonFile['titulacion']:
            simulacion['titulacion']['titulacion_' +
                                     str(tit)][campo] = eval(jsonFile['titulacion'][campo]['funcion'])
...
def asig_por_prof():
    ...
    for n_asig, asig in simulacion['asignatura'].items():
        if asig['profesor'] is None:
            asig_posibles.add(n_asig)

    calc = len(simulacion['asignatura'])/len(prof_actual)
    n_asig_prof = int(calc if (calc).is_integer() else calc+1)
...
def asig_por_alum():
    ...
    for alumname, alum in simulacion['alumno'].items():
        if alum['aban'] == False:
            ...
            while it < len(titut['asignaturas']) and len(alum['asig_mat']) < n_asignaturas/titut['num_cursos']:
